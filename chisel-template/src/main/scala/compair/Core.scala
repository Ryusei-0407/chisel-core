...
class Core extends Module {
  ...
  val alu_out = MuxCase(0.U(WORD_LEN.W), Seq(
    ...
    (exe_fun === ALU_SLL) -> (op1_data << op2_data(4, 0))(31, 0), // SLL, SLLI
    (exe_fun === ALU_SRL) -> (op1_data >> op2_data(4, 0)).asUInt(), // SRL, SRLI
    (exe_fun === ALU_SRA) -> (op1_data.asSInt() >> op2_data(4, 0)).asUInt() // SRA, SRAI
    (exe_fun === ALU_SLT) -> (op1_data.asSInt() < op2_data.asSInt()).asUInt(), // SLT, SLTI
    (exe_fun === ALU_SLTU) -> (op1_data < op2_data).asUInt() // SLTU, SLTIU
  ))
